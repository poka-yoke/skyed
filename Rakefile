...
task :release do
  ...
  pub_branch = 'master'
  pub_branch = 'v0.2' if /0\.2/ =~ new_version
  ...
  repo = Git.open('.')
  current_branch = pub_branch
  fail "Switch to #{pub_branch} and merge" unless current_branch == pub_branch
  ...
  puts 'Pushing'
  repo.push(repo.remote('ifosch'), pub_branch, tags: true) unless ENV['FAKE']
